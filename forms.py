...
from wtforms import BooleanField, HiddenField, PasswordField, SubmitField, StringField, SelectField, IntegerField
...
class RegisterForm(FlaskForm):
    ...
    email = StringField(_('Email'), validators=[
        ...
        validators.Email(_('Invalid Email')),
        unique_email_validator])
    firstname = StringField(_('First Name'), validators=[
        validators.DataRequired(_('First Name is required'))])
    lastname = StringField(_('Last Name'), validators=[
        validators.DataRequired(_('Last Name is required'))])
    dateofbirth = DateField(_('Date Of Birth'), validators=[
        validators.DataRequired(_('Date Of Birth is required'))])
    phonenumber = IntegerField(_('Your Primary Phone Number'), validators=[
        validators.DataRequired(_('Phone number is required'))])
    address = StringField(_('Home Address'), validators=[
        validators.DataRequired(_('Home Address is required'))])
    parents_phonenumber = IntegerField(_('Parent Phone Number'), validators=[
        validators.DataRequired(_('Parents Phone Number is required'))])
    blood_group = SelectField(_('Blood Group'), choices=[('1', 'A+'), ('2','O+'), ('3', 'B+'),('4', 'AB+'), ('5','A-'), ('6', 'O-'),('7','B-'), ('8', 'AB-'), ('9', 'Dont Know')],  validators=[
        validators.DataRequired(_('Blood group is needed' ))])
    hostel = SelectField(_('Hostel'), validators=[
        validators.DataRequired(_('Hostel is required'))], coerce=int, choices=[()])
    password = PasswordField(_('Password'), validators=[
        validators.DataRequired(_('Password is required')),
        password_validator])
