...
class UserManager__Views(object):
    ...
    def register_view(self):
        ...
        #registerformchoices
        register_form.hostel.choices = [(hostel.id, hostel.hostel_name) for
                                        hostel in self.db_manager.find_hostel_by_hostelname()]
        ...
        if request.method == 'POST' and register_form.validate():
            #[Nizam] Why no parameters passed for user, like user details, because in line 431, we are accessing the registartion form as request.form
            # link https://scotch.io/bar-talk/processing-incoming-request-data-in-flask
            # all the user attributes are committed here
            user = self.db_manager.add_user()
            ...
            user.phonenumber = register_form.phonenumber.data
            user.first_name = register_form.firstname.data
            user.last_name = register_form.lastname.data
            user.dob = register_form.dateofbirth.data
            user.address = register_form.address.data
            user.parent_phone = register_form.parents_phonenumber.data
            user.blood_group = dict(register_form.blood_group.choices).get(register_form.blood_group.data)
            user.hostel_location = dict(register_form.hostel.choices).get(register_form.hostel.data)
            ...
            self.db_manager.save_user_details(user, user_email)
            self.db_manager.commit()
